...
async function loadSummary() {
  if (!currentProjectId) return;
  const [{ keywords, dailyVisibility }, monthly, newlyAppeared] = await Promise.all([
...
function renderViewRow(tr, k) {
  ...
  let rowHtml = `<td><span class="term-link" data-action="open-detail">${k.term}</span>${medalla}</td>`;
  COLUMN_DEFS.forEach((col) => {
...
function renderEditRow(tr, k) {
  ...
  const totalCols = 2 + COLUMN_DEFS.filter((c) => visibleColumns[c.key]).length;
  tr.innerHTML = `
