...
const groupByTagToggle = document.getElementById("groupByTagToggle");
let groupByTag = false;
let expandedGroups = new Set();
groupByTagToggle.addEventListener("change", () => {
  groupByTag = groupByTagToggle.checked;
  renderKeywordsTable();
});
...
document.getElementById("bulkSubmitBtn").addEventListener("click", async (e) => {
  ...
  const ai_provider = document.getElementById("bulkProvider").value;
  const tags = document
  ...
  try {
    const resultado = await api(`/api/projects/${currentProjectId}/keywords/bulk`, {
      method: "POST",
      body: JSON.stringify({ terms, city, country, tags, ai_provider }),
    });
...
async function loadProjects() {
  ...
  if (!currentProjectId && projects.length) {
    const savedIdRaw = localStorage.getItem("radar_current_project_id");
    const savedId = savedIdRaw ? Number(savedIdRaw) : null;
    const proyectoGuardado = savedId ? projects.find((p) => p.id === savedId) : null;
    const proyectoAUsar = proyectoGuardado || projects[0];
    selectProject(proyectoAUsar.id, proyectoAUsar.name, proyectoAUsar.domain, proyectoAUsar.frequency_days);
  }
}
...
function renderProjectView(div, p) {
  ...
  div.querySelector(".del").addEventListener("click", async (e) => {
    ...
    if (currentProjectId === p.id) {
      ...
      localStorage.removeItem("radar_current_project_id");
...
async function selectProject(id, name, domain, frequencyDays) {
  currentProjectId = id;
  localStorage.setItem("radar_current_project_id", String(id));
...
function grupoKeyDeTerm(k) {
  const tags = (k.tags || []).slice().sort();
  return tags.length ? tags.join(" | ") : "__sin_etiqueta__";
}

function renderGroupedRows(visibles) {
  const totalCols = 3 + COLUMN_DEFS.filter((c) => visibleColumns[c.key]).length;

  const grupos = new Map();
  visibles.forEach((k) => {
    const key = grupoKeyDeTerm(k);
    if (!grupos.has(key)) grupos.set(key, []);
    grupos.get(key).push(k);
  });

  const clavesOrdenadas = [...grupos.keys()].sort((a, b) => {
    if (a === "__sin_etiqueta__") return 1;
    if (b === "__sin_etiqueta__") return -1;
    return a.localeCompare(b);
  });

  clavesOrdenadas.forEach((key) => {
    const miembros = grupos.get(key);
    const expandido = expandedGroups.has(key);
    const tagsHtml =
      key === "__sin_etiqueta__"
    ...
    trGrupo.innerHTML = `
      ...
      <td colspan="${totalCols - 1}">
        <span class="group-row-arrow ${expandido ? "expanded" : ""}">▸</span>
        ${tagsHtml}
        <span class="group-row-count">${miembros.length}</span>
      </td>`;
    ...
    keywordsBody.appendChild(trGrupo);
...
function renderViewRow(tr, k) {
  ...
  const proveedorBadge =
    k.ai_provider === "openai"
      ? `<span class="provider-badge provider-openai" title="Este término usa OpenAI directo">OpenAI</span>`
      : k.ai_provider === "openrouter"
      ? `<span class="provider-badge provider-openrouter" title="Este término usa OpenRouter">Router</span>`
      : "";

  let rowHtml = `<td><input type="checkbox" class="row-select" ${selectedKeywordIds.has(k.keyword_id) ? "checked" : ""} /></td>`;
  rowHtml += `<td><span class="term-link" data-action="open-detail">${k.term}</span>${medalla}${proveedorBadge}</td>`;
  COLUMN_DEFS.forEach((col) => {
...
function renderEditRow(tr, k) {
  ...
  tr.innerHTML = `
    <td colspan="${totalCols}">
      <div class="edit-inline-form">
        ...
        <select class="edit-provider">
          <option value="" ${!k.ai_provider ? "selected" : ""}>Automático</option>
          <option value="openai" ${k.ai_provider === "openai" ? "selected" : ""}>OpenAI</option>
          <option value="openrouter" ${k.ai_provider === "openrouter" ? "selected" : ""}>OpenRouter</option>
        </select>
        <button type="button" data-action="save">Guardar</button>
        <button type="button" data-action="cancel">Cancelar</button>
      </div>
  ...
  tr.querySelector("[data-action='save']").addEventListener("click", async () => {
    ...
    const ai_provider = tr.querySelector(".edit-provider").value;
    if (!term) return;
    try {
      await api(`/api/keywords/${k.keyword_id}`, {
        method: "PUT",
        body: JSON.stringify({ term, tags, city, country, ai_provider }),
      });
      await loadSummary();
    } catch (err) {
      alert(err.message);
...
document.getElementById("newKeywordForm").addEventListener("submit", async (e) => {
  ...
  const ai_provider = document.getElementById("newKeywordProvider").value;
  ...
  try {
    await api(`/api/projects/${currentProjectId}/keywords`, {
      method: "POST",
      body: JSON.stringify({ term, city, country, tags, ai_provider }),
    });
    e.target.reset();
    await loadSummary();
  } catch (err) {
