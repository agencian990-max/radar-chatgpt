...
async function loadSummary() {
  ...
  populateTagFilter(keywords);
  renderKeywordsTable();
  renderChart(dailyVisibility);
}

function populateTagFilter(keywords) {
  const tagsActuales = new Set(keywords.map((k) => k.tag).filter(Boolean));
  const seleccionPrevia = tagFilterSelect.value;
  ...
  // Mantener la selección si esa etiqueta sigue existiendo
  if ([...tagsActuales].includes(seleccionPrevia)) tagFilterSelect.value = seleccionPrevia;
}

function renderKeywordsTable() {
  const filtro = tagFilterSelect.value;
  const visibles = filtro ? keywordsCache.filter((k) => k.tag === filtro) : keywordsCache;
...
function renderViewRow(tr, k) {
  ...
  const tagHtml = k.tag ? `<span class="tag-pill">${k.tag}</span>` : `<span class="muted">—</span>`;
  tr.innerHTML = `
    <td>${k.term}</td>
    <td>${tagHtml}</td>
...
function renderEditRow(tr, k) {
  tr.innerHTML = `
    ...
    <td><input type="text" class="edit-tag" placeholder="Etiqueta" value="${k.tag || ""}" style="width:100px;" /></td>
    <td colspan="2">
  ...
  tr.querySelector("[data-action='save']").addEventListener("click", async () => {
    ...
    const tag = tr.querySelector(".edit-tag").value.trim();
    ...
    await api(`/api/keywords/${k.keyword_id}`, {
      method: "PUT",
      body: JSON.stringify({ term, tag, city, country }),
    });
    await loadSummary();
  });
}
...
document.getElementById("newKeywordForm").addEventListener("submit", async (e) => {
  ...
  const tag = document.getElementById("newKeywordTag").value.trim();
  if (!term) return;
  await api(`/api/projects/${currentProjectId}/keywords`, {
    method: "POST",
    body: JSON.stringify({ term, city, country, tag }),
  });
  e.target.reset();
  await loadSummary();
});
