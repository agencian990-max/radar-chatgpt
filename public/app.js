...
let currentProjectPaused = false;
...
async function loadProjects() {
  ...
  if (!currentProjectId && projects.length) {
    selectProject(projects[0].id, projects[0].name, projects[0].domain, projects[0].frequency_days, projects[0].is_paused);
  }
}

function renderProjectView(div, p) {
  div.classList.toggle("is-paused", !!p.is_paused);
  const pausedBadge = p.is_paused ? `<span class="paused-badge">⏸ PAUSADO</span>` : "";
  div.innerHTML = `<span class="proj-name">${p.name}${pausedBadge}</span><span class="proj-actions"><span class="edit" data-id="${p.id}">✎</span><span class="del" data-id="${p.id}">✕</span></span>`;
  div.addEventListener("click", (e) => {
    if (e.target.classList.contains("del") || e.target.classList.contains("edit")) return;
    selectProject(p.id, p.name, p.domain, p.frequency_days, p.is_paused);
  });
...
async function selectProject(id, name, domain, frequencyDays, isPaused) {
  currentProjectId = id;
  ...
  updatePausedUI(!!isPaused);
  await loadProjects();
  await loadSummary();
}

function updatePausedUI(isPaused) {
  currentProjectPaused = isPaused;
  pauseToggleBtn.textContent = isPaused ? "▶ Reanudar rastreo" : "⏸ Pausar rastreo";
...
pauseToggleBtn.addEventListener("click", async () => {
  if (!currentProjectId) return;
  const nuevoEstado = !currentProjectPaused;
  pauseToggleBtn.disabled = true;
  try {
    const updated = await api(`/api/projects/${currentProjectId}`, {
      method: "PUT",
      body: JSON.stringify({ is_paused: nuevoEstado }),
    });
    updatePausedUI(updated.is_paused);
    await loadProjects();
  } catch (err) {
    alert("No se pudo actualizar la pausa: " + err.message);
  } finally {
