...
app.post("/api/projects/:id/keywords", async (req, res) => {
  const { term, city, region, country } = req.body;
  if (!term) return res.status(400).json({ error: "Falta term" });
  const { rows } = await pool.query(
    `INSERT INTO keywords (project_id, term, city, region, country)
     VALUES ($1, $2, $3, $4, $5) RETURNING *`,
    [req.params.id, term, city || null, region || null, country || null]
  );
  res.json(rows[0]);
});
...
app.put("/api/keywords/:id", async (req, res) => {
  ...
  if (!term) return res.status(400).json({ error: "Falta term" });
  const { rows } = await pool.query(
    `UPDATE keywords SET term = $1, city = $2, region = $3, country = $4
     WHERE id = $5 RETURNING *`,
    [term, city || null, region || null, country || null, req.params.id]
  );
  res.json(rows[0]);
});
...
app.post("/api/keywords/:id/check", async (req, res) => {
  ...
  const inserted = await pool.query(
    `INSERT INTO checks (keyword_id, status, sources) VALUES ($1, $2, $3) RETURNING *`,
...
app.post("/api/check-all", async (req, res) => {
  ...
  runAllChecks().catch((err) => console.error("[check-all] Error:", err));
});
...
app.get("/api/projects/:id/summary", async (req, res) => {
  ...
  const latest = await pool.query(
    `SELECT DISTINCT ON (k.id) k.id AS keyword_id, k.term, k.city, k.region, k.country,
