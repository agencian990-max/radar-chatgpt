...
app.post("/api/projects/:id/keywords", async (req, res) => {
  const { term, city, region, country, tag } = req.body;
  if (!term) return res.status(400).json({ error: "Falta term" });
  const { rows } = await pool.query(
    `INSERT INTO keywords (project_id, term, city, region, country, tag)
     VALUES ($1, $2, $3, $4, $5, $6) RETURNING *`,
    [req.params.id, term, city || null, region || null, country || null, tag || null]
  );
  res.json(rows[0]);
});
...
app.put("/api/keywords/:id", async (req, res) => {
  ...
  if (!term) return res.status(400).json({ error: "Falta term" });
  const { rows } = await pool.query(
    `UPDATE keywords SET term = $1, city = $2, region = $3, country = $4, tag = $5
     WHERE id = $6 RETURNING *`,
    [term, city || null, region || null, country || null, tag || null, req.params.id]
  );
  res.json(rows[0]);
});
...
app.get("/api/projects/:id/summary", async (req, res) => {
  ...
  const latest = await pool.query(
    `SELECT DISTINCT ON (k.id) k.id AS keyword_id, k.term, k.city, k.region, k.country, k.tag,
