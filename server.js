...
app.post("/api/projects", async (req, res) => {
  const { name, domain, frequency_days, is_paused } = req.body;
  if (!name || !domain) return res.status(400).json({ error: "Falta name o domain" });
  const { rows } = await pool.query(
    "INSERT INTO projects (name, domain, frequency_days, is_paused) VALUES ($1, $2, $3, $4) RETURNING *",
    [name, domain, frequency_days || 1, is_paused === true]
  );
  res.json(rows[0]);
});
...
app.put("/api/projects/:id", async (req, res) => {
  const { name, domain, frequency_days, is_paused } = req.body;
  // is_paused es booleano, no se puede usar "|| null" (perdería el valor "false"),
  // así que se distingue explícitamente "no vino en el body" (undefined) de "false".
  const isPausedParam = typeof is_paused === "boolean" ? is_paused : null;
  const { rows } = await pool.query(
    `UPDATE projects SET
    ...
    [name || null, domain || null, frequency_days || null, isPausedParam, req.params.id]
  );
  res.json(rows[0]);
});
...
app.post("/api/projects/:id/keywords", async (req, res) => {
  ...
  const dup = await pool.query(
    `SELECT id FROM keywords WHERE project_id = $1 AND lower(trim(term)) = lower(trim($2))`,
    [req.params.id, term]
  );
  if (dup.rows.length) {
    return res.status(409).json({ error: `Ese término ya existe en este proyecto: "${term.trim()}"` });
  }
...
app.post("/api/projects/:id/keywords/bulk", async (req, res) => {
  ...
  const { rows: existentes } = await pool.query(
    `SELECT lower(trim(term)) AS t FROM keywords WHERE project_id = $1`,
    [req.params.id]
  );
  const existentesSet = new Set(existentes.map((r) => r.t));
...
app.put("/api/keywords/:id", async (req, res) => {
  ...
  const dup = await pool.query(
    `SELECT id FROM keywords WHERE project_id = $1 AND lower(trim(term)) = lower(trim($2)) AND id <> $3`,
    [projectId, term, req.params.id]
  );
  if (dup.rows.length) {
    return res.status(409).json({ error: `Ese término ya existe en este proyecto: "${term.trim()}"` });
  }
...
app.post("/api/keywords/:id/check", async (req, res) => {
  const { rows } = await pool.query(
    `SELECT k.*, p.domain, p.is_paused FROM keywords k JOIN projects p ON p.id = k.project_id WHERE k.id = $1`,
    [req.params.id]
  );
  const kw = rows[0];
  if (!kw) return res.status(404).json({ error: "Keyword no encontrada" });
  if (kw.is_paused) {
    return res.status(409).json({ error: "El rastreo de este proyecto está pausado. Reanúdalo para poder revisar términos." });
  }
