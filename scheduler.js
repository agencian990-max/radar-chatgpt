...
async function runAllChecks({ forzarTodo = false } = {}) {
  const { rows: keywords } = await pool.query(`
    SELECT k.id, k.term, k.city, k.region, k.country, p.domain, p.frequency_days, p.is_paused,
  ...
  const pendientes = keywords.filter((kw) => {
    // Un proyecto pausado nunca se revisa -- ni siquiera con forzarTodo (botón "Revisar ahora").
    if (kw.is_paused) return false;
  ...
  const pausadas = keywords.filter((kw) => kw.is_paused).length;
  console.log(
    `[scheduler] ${pendientes.length} de ${keywords.length} keywords a revisar (forzarTodo=${forzarTodo}, ${pausadas} omitidas por proyecto pausado).`
  );
...
const inserted = await pool.query(
      `INSERT INTO checks (keyword_id, status, sources, response_text, own_position) VALUES ($1, $2, $3, $4, $5) RETURNING checked_at`,
